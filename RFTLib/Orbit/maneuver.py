...
def hohmann_circular_velocities(mu: float, peri_radius: float, apo_radius: float) -> (float, float, float, float):
    """
    Calculate the velocity change required for the entire hohmann transfer from origin to target orbit, assuming
    circular origin and target orbit

    :param mu: [km³/s²] Standard gravitational parameter of the reference object
    :param peri_radius: [km] Radius (distance) of the object in the origin orbit's peri-center
    :param apo_radius: [km] Radius (distance) of the object in the target orbit's apo-center
    :return: dv [km/s] Velocity change required for the entire hohmann transfer
    """

    transfer_a = hohmann_transfer_a(peri_radius, apo_radius)

    return delta(*hohmann_circular_velocities_peri(mu, peri_radius, transfer_a)) + \
        delta(*hohmann_circular_velocities_apo(mu, transfer_a, apo_radius))
...
def hohmann_velocities(mu: float, origin_a: float, peri_radius: float,
    ...
    """
    Calculate the velocity change required for the entire hohmann transfer from origin to target orbit
    ...
    return delta(*hohmann_velocities_peri(mu, origin_a, peri_radius, transfer_a)) + \
        delta(*hohmann_velocities_apo(mu, transfer_a, target_a, apo_radius))
